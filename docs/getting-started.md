...
本文按当前脚本依赖顺序整理。已在已有开发环境校对命令和模型缓存；**尚未在完全空白的第二台机器执行全部步骤**。遇到平台差异请附脱敏错误，不上传配置文件。
...
以下命令均从仓库根目录执行，除非明确写了`Push-Location`。不要把父级工作目录初始化成仓库，也不要把示例密钥照搬进运行配置。
...
导入源、数量和图片说明见[数据与测试](testing.md)、[商品目录记录](28-product-catalog.md)、[政策库记录](16-policy-library-v2.md)。
...
需要完整前置数据；测试会写入合成账号、订单和事件。MCP/混合检索验证单独见[验收记录](34-v1-acceptance.md)。这不是必须每次启动都跑的步骤。
