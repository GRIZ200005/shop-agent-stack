...
页面不保存密钥，不向客户端公开基础地址。适配层关闭重定向及环境代理自动继承；OpenAI 请求 `store=false`，不同供应商数据保留政策并不由这一参数统一控制。工具数据、问题和对话历史会发送到所选服务商；默认使用合成账户/订单进行学习。
...
学习时可沿一个确认请求追踪 React → REST → Agent → MCP → Java 事务 → MySQL，再分别深入 HTTP、Spring 事务/行锁、Python async、幂等与事件恢复；能解释框架图之后，还需能读懂并独立修改关键实现。
