...
export const marker = 'aster-catalog-v1-original-synthetic';
export const promptPrefix = 'Use case: product-mockup. Create one square individual product catalog photograph for Aster Commerce, a fictional demo store. ';
...
export function loadCatalog({ requireImages = false } = {}) {
  ...
  const products = lines.map((line, index) => {
    ...
    return { ...row, id: 10001 + index, skuId: 10001 + index, categoryId: 1011 + categories.indexOf(row.category),
      brandId: 101, productSn: `ASTER-LIFE-${row.slug.toUpperCase()}`, pic,
