...
try {
    ...
    docker run --rm --network aster-p0_default -e ASTER_INTEGRATION=true --mount "type=bind,source=$root/services/agent,target=/app,readonly" aster-agent:p2 python -m pytest tests -q -p no:cacheprovider
