...
from aster_agent.business import java
...
async def main():
    ...
    async with httpx.AsyncClient(base_url="http://admin:8080",timeout=20) as client:
        ...
        if sys.argv[1]=="prepare":
            ...
            sale=await java("/aster/after-sales",bearer=bearer,body={"orderId":oid,"reason":"P4 isolated recovery verification"})
            cid=sale["id"]
            claims=await asyncio.gather(*(client.post(f"/aster/after-sales/{cid}/claim",json={}) for _ in range(2)))
            ...
            decisions=await asyncio.gather(*(client.post(f"/aster/after-sales/{cid}/decision",json={"approved":True,"note":"P4 MQ outage verification"}) for _ in range(2)))
            assert sum(r.json()["code"]==200 for r in decisions)==1,"Concurrent approval must have one winner"
            sale=(await client.get(f"/aster/after-sales/{cid}")).json()["data"]
