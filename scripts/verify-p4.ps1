...
function Query([string]$sql) {
    $result=$sql | docker @compose exec -T mysql sh -c 'MYSQL_PWD="$MYSQL_PASSWORD" mysql -uaster -Daster -N'
...
function Fixture([string]$phase) {
    docker run --rm --network aster-p0_default --mount "type=bind,source=$root,target=/workspace,readonly" --mount "type=bind,source=$root/.local,target=/workspace/.local" --workdir /workspace aster-agent:p2 python scripts/refund-fixture.py $phase
...
try {
    ...
    try {
        ...
        Fixture approve
        $before=Query "SELECT COUNT(*) FROM aster_refund_job WHERE case_id=$caseId AND status='PENDING'; SELECT COUNT(*) FROM aster_simulated_refund WHERE case_id=$caseId; SELECT status FROM oms_order WHERE id=$orderId;"
    ...
    $done=$false
    for($i=0;$i -lt 45;$i++) {
        $state=Query "SELECT status FROM aster_refund_job WHERE case_id=$caseId;"
    ...
    $publish='rabbitmqadmin -u "$RABBITMQ_DEFAULT_USER" -p "$RABBITMQ_DEFAULT_PASS" -V /aster publish exchange=amq.default routing_key=aster.refunds.v1 payload='+$caseId
    1..2 | ForEach-Object {
    ...
    for($i=0;$i -lt 15;$i++) {
        Start-Sleep -Seconds 2
        $raw=docker @compose exec -T rabbitmq sh -c 'rabbitmqadmin -u "$RABBITMQ_DEFAULT_USER" -p "$RABBITMQ_DEFAULT_PASS" -V /aster -f raw_json list queues name messages_ready messages_unacknowledged'
        if($LASTEXITCODE -ne 0){throw 'Cannot verify queue drain'}
        $queue=($raw | ConvertFrom-Json) | Where-Object {$_.name -eq 'aster.refunds.v1'}
    ...
    $after=Query "SELECT COUNT(*) FROM aster_simulated_refund WHERE case_id=$caseId AND order_id=$orderId; SELECT COUNT(*) FROM aster_after_sale_event WHERE case_id=$caseId AND action='REFUNDED'; SELECT COUNT(*) FROM aster_after_sale a JOIN oms_order o ON o.id=a.order_id JOIN aster_refund_job j ON j.case_id=a.id WHERE a.id=$caseId AND a.status='REFUNDED' AND o.status=4 AND j.status='DONE' AND a.amount=o.pay_amount;"
