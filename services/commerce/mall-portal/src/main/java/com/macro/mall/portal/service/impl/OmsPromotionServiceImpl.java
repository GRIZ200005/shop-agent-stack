    ...
    public List<CartPromotionItem> calcCartPromotion(List<OmsCartItem> cartItemList) {
        ...
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            if (promotionProduct == null) {
                Asserts.fail("Product not available");
            }
            for (OmsCartItem item : itemList) {
                if (item.getQuantity() == null || item.getQuantity() <= 0 || item.getProductSkuId() == null) {
                    Asserts.fail("Invalid cart quantity or SKU");
                }
                PmsSkuStock sku = getOriginalPrice(promotionProduct, item.getProductSkuId());
                if (sku == null || sku.getPrice() == null || sku.getPrice().signum() < 0) {
                    Asserts.fail("SKU not available");
                }
                // Cart prices are client supplied; all promotion branches must start from catalog prices.
