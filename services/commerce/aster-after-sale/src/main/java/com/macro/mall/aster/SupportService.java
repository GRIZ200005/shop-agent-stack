...
/** Original Aster human support. Customer supplied context is never business authority. */
@Service
public class SupportService {
    ...
    private Map<String,Object> ticket(String id,Long member,boolean lock){
        var rows=db.queryForList("SELECT * FROM aster_support_ticket WHERE id=?"+(member==null?"":" AND member_id=?")+(lock?" FOR UPDATE":""),member==null?new Object[]{uuid(id)}:new Object[]{uuid(id),member});
    ...
    public Map<String,Object> list(Long member,int page){
        ...
        String sql="SELECT id,title,status,assignee_id,created_at,updated_at FROM aster_support_ticket"+(member==null?"":" WHERE member_id=?")+" ORDER BY created_at DESC,id DESC LIMIT 21 OFFSET ?";
    ...
    public Map<String,Object> detail(String id,Long member){
        ...
        result.put("messages",db.queryForList("SELECT id,author_role,content,created_at FROM aster_support_message WHERE ticket_id=? ORDER BY seq LIMIT 200",id));
        return result;
    }
    @Transactional
    public Map<String,Object> create(long member,String requestId,String title,String context){
        ...
        try{db.update("INSERT INTO aster_support_ticket(id,member_id,title,context_text) VALUES(?,?,?,?)",id,member,name,excerpt);}
    ...
    public void claim(String id,long staff){
        ...
        db.update("UPDATE aster_support_ticket SET status='IN_PROGRESS',assignee_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",staff,id);
    ...
    private void system(String id,long staff,String content){db.update("INSERT INTO aster_support_message(id,ticket_id,author_id,author_role,content) VALUES(?,?,?,'SYSTEM',?)",UUID.randomUUID().toString(),id,staff,content);}
    @Transactional
    public void reply(String id,long actor,boolean staff,String requestId,String content){
        ...
        var prior=db.queryForList("SELECT * FROM aster_support_message WHERE id=?",mid);
        if(!prior.isEmpty()){
            var p=prior.get(0);
        ...
        if(db.queryForObject("SELECT COUNT(*) FROM aster_support_message WHERE ticket_id=?",Long.class,id)>=190)Asserts.fail("本咨询消息已达上限，请结束后新建咨询");
        db.update("INSERT INTO aster_support_message(id,ticket_id,author_id,author_role,content) VALUES(?,?,?,?,?)",mid,id,actor,role,body);
        db.update("UPDATE aster_support_ticket SET updated_at=CURRENT_TIMESTAMP WHERE id=?",id);
    }
    @Transactional
    public void resolve(String id,long staff){
        ...
        if(db.queryForObject("SELECT COUNT(*) FROM aster_support_message WHERE ticket_id=? AND author_role='STAFF'",Long.class,id)==0)Asserts.fail("请先回复客户再结束咨询");
        db.update("UPDATE aster_support_ticket SET status='RESOLVED',updated_at=CURRENT_TIMESTAMP WHERE id=?",id);
