...
@EnableRabbit
@EnableScheduling
@ConditionalOnProperty(name="aster.refund.enabled",havingValue="true")
public class RefundWorker {
    public static final String QUEUE="aster.refunds.v1";
    ...
    @Scheduled(fixedDelayString="${aster.refund.poll-ms:2000}")
    public void dispatch() {
        for(var row:db.queryForList("SELECT case_id,attempts FROM aster_refund_job WHERE status='PENDING' AND next_attempt_at<=NOW() ORDER BY next_attempt_at LIMIT 10")) {
            ...
            if(db.update("UPDATE aster_refund_job SET attempts=attempts+1,next_attempt_at=TIMESTAMPADD(SECOND,30,NOW()) WHERE case_id=? AND status='PENDING' AND next_attempt_at<=NOW() AND attempts<20",id)!=1) continue;
            try {
                ...
                db.update("UPDATE aster_refund_job SET last_error='PUBLISH_UNCONFIRMED' WHERE case_id=? AND status='PENDING'",id);
            }
        }
    }
