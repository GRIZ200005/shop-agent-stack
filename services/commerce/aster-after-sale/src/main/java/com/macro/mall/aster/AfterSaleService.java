...
public class AfterSaleService {
    ...
    public Map<String,Object> submit(long memberId, long orderId, String reason) {
        ...
        var existing = db.queryForList("SELECT * FROM aster_after_sale WHERE order_id=?", orderId);
        ...
        db.update("INSERT INTO aster_after_sale(order_id,member_id,reason,amount) VALUES(?,?,?,?)", orderId,memberId,reason.trim(),amount);
        long id = db.queryForObject("SELECT id FROM aster_after_sale WHERE order_id=?",Long.class,orderId);
        event(id,"customer:"+memberId,"SUBMITTED","客户提交售后申请");
        return detail(id,memberId);
    }
    public List<Map<String,Object>> mine(long memberId) { return db.queryForList("SELECT * FROM aster_after_sale WHERE member_id=? ORDER BY id DESC LIMIT 100",memberId); }
    public Map<String,Object> detail(long id, Long memberId) {
        var rows = memberId == null ? db.queryForList("SELECT * FROM aster_after_sale WHERE id=?",id) : db.queryForList("SELECT * FROM aster_after_sale WHERE id=? AND member_id=?",id,memberId);
        if (rows.isEmpty()) Asserts.fail("售后不存在或无权访问");
        var result=rows.get(0);
        result.put("events",db.queryForList("SELECT action,note,created_at FROM aster_after_sale_event WHERE case_id=? ORDER BY id",id));
        return result;
    }
    public List<Map<String,Object>> queue() { return db.queryForList("SELECT a.*,u.nick_name assignee_name FROM aster_after_sale a LEFT JOIN ums_admin u ON u.id=a.assignee_id ORDER BY a.id DESC LIMIT 100"); }
    @Transactional
    public void claim(long id,long staffId) {
        if (db.update("UPDATE aster_after_sale SET status='CLAIMED',assignee_id=? WHERE id=? AND status='SUBMITTED' AND assignee_id IS NULL",staffId,id)!=1) Asserts.fail("工单已被领取或状态已更新，请刷新");
