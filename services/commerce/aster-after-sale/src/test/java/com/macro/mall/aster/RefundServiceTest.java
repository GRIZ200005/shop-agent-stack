...
class RefundServiceTest {
    ...
    @BeforeEach void setup() {
        ...
        db.execute("CREATE TABLE aster_after_sale(id BIGINT PRIMARY KEY,order_id BIGINT,amount DECIMAL(10,2),status VARCHAR(24),assignee_id BIGINT,decision_note VARCHAR(500),refund_reference VARCHAR(80))");
        db.execute("CREATE TABLE aster_refund_job(case_id BIGINT PRIMARY KEY,status VARCHAR(20) DEFAULT 'PENDING',attempts INT DEFAULT 0,next_attempt_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,last_error VARCHAR(80))");
        db.execute("CREATE TABLE aster_simulated_refund(case_id BIGINT PRIMARY KEY,order_id BIGINT UNIQUE,amount DECIMAL(10,2),reference VARCHAR(80) UNIQUE)");
        db.execute("CREATE TABLE aster_after_sale_event(case_id BIGINT,actor VARCHAR(80),action VARCHAR(40),note VARCHAR(500))");
        db.update("INSERT INTO oms_order VALUES(1,1,49.90,NULL)");
        db.update("INSERT INTO aster_after_sale VALUES(7,1,49.90,'CLAIMED',9,NULL,NULL)");
    }
    @Test void approvalOnlyQueuesUntilWorkerCommits() {
        sales.decide(7,9,true,"approved");
        assertEquals("REFUNDING",db.queryForObject("SELECT status FROM aster_after_sale",String.class));
        assertEquals(1,db.queryForObject("SELECT status FROM oms_order",Integer.class));
        refunds.complete(7);
        assertEquals("DONE",db.queryForObject("SELECT status FROM aster_refund_job",String.class));
    ...
    @Test void outboxInsertFailureRollsBackApproval() {
        db.update("INSERT INTO aster_refund_job(case_id) VALUES(7)");
        assertThrows(RuntimeException.class,()->sales.decide(7,9,true,"approved"));
        assertEquals("CLAIMED",db.queryForObject("SELECT status FROM aster_after_sale",String.class));
        assertEquals(0,db.queryForObject("SELECT COUNT(*) FROM aster_after_sale_event",Integer.class));
    }
    @Test void concurrentDuplicateDeliveryAppliesOnlyOnce() throws Exception {
        ...
        assertEquals(1,db.queryForObject("SELECT COUNT(*) FROM aster_simulated_refund",Integer.class));
        assertEquals(1,db.queryForObject("SELECT COUNT(*) FROM aster_after_sale_event WHERE action='REFUNDED'",Integer.class));
    }
    @Test void changedAmountCannotProduceRefund() {
        ...
        assertEquals(0,db.queryForObject("SELECT COUNT(*) FROM aster_simulated_refund",Integer.class));
        assertEquals("PENDING",db.queryForObject("SELECT status FROM aster_refund_job",String.class));
    }
    @Test void exhaustedJobNeedsOwnerRetryAndLateMessageCannotBypassIt() {
        sales.decide(7,9,true,"approved"); db.update("UPDATE aster_refund_job SET attempts=20");
        refunds.exhaust(7); refunds.complete(7);
        assertEquals("REFUND_REVIEW",db.queryForObject("SELECT status FROM aster_after_sale",String.class));
    ...
    @Test void reconcileDoesNotTreatPendingOrLegacyAsCorruption() {
        ...
        db.update("UPDATE aster_after_sale SET status='REFUNDED'");
        assertEquals("LEGACY_NOT_TRACKED",diagnostics.check(7).get("reconciliation"));
        db.update("UPDATE aster_after_sale SET status='CLAIMED'"); sales.decide(7,9,true,"approved");
    ...
    @Test void reconciliationDetectsMissingLedgerWrongAmountAndDuplicateEvent() {
        ...
        db.update("UPDATE aster_simulated_refund SET amount=1");
        assertEquals("MISMATCH",diagnostics.check(7).get("reconciliation"));
        db.update("UPDATE aster_simulated_refund SET amount=49.90");
        db.update("INSERT INTO aster_after_sale_event VALUES(7,'test','REFUNDED','duplicate')");
        assertEquals("MISMATCH",diagnostics.check(7).get("reconciliation"));
        db.update("DELETE FROM aster_simulated_refund");
        assertEquals("MISMATCH",diagnostics.check(7).get("reconciliation"));
        assertEquals("REFUNDED",db.queryForObject("SELECT status FROM aster_after_sale",String.class));
    }
    @Test void completedTaskCannotBeOverwrittenByFailureRecording() {
        ...
        assertEquals("BUSINESS_STATE_CONFLICT",db.queryForObject("SELECT last_error FROM aster_refund_job",String.class));
        refunds.complete(7); refunds.recordFailure(7,"CONSUMER_FAILURE");
        assertNull(db.queryForObject("SELECT last_error FROM aster_refund_job",String.class));
    }
}
