...
class SupportServiceTest {
 ...
 @BeforeEach void setup(){
  ...
  db.execute("CREATE TABLE aster_support_ticket(id VARCHAR(36) PRIMARY KEY,member_id BIGINT,title VARCHAR(120),context_text TEXT,status VARCHAR(20) DEFAULT 'WAITING',assignee_id BIGINT,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
  db.execute("CREATE TABLE aster_support_message(seq BIGINT AUTO_INCREMENT UNIQUE,id VARCHAR(36) PRIMARY KEY,ticket_id VARCHAR(36),author_id BIGINT,author_role VARCHAR(16),content TEXT,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
 }
 @Test void createIsIdempotentAndOwnerScoped(){
  service.create(1,id,"规格问题","客户提供的摘录");service.create(1,id,"规格问题","客户提供的摘录");
  assertEquals(1,db.queryForObject("SELECT COUNT(*) FROM aster_support_ticket",Integer.class));
 ...
 @Test void repliesClaimsAndResolutionRespectBoundaries(){
  ...
  assertEquals(1,db.queryForObject("SELECT COUNT(*) FROM aster_support_message WHERE author_role='STAFF'",Integer.class));
