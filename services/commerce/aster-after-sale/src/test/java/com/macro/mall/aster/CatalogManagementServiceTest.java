...
class CatalogManagementServiceTest {
 ...
 @BeforeEach void setup(){
  ...
  db.execute("CREATE TABLE aster_catalog_change(request_id VARCHAR(36) PRIMARY KEY,product_id BIGINT,sku_id BIGINT,actor_id BIGINT,action VARCHAR(16),expected_value INT,requested_value INT,before_value INT,after_value INT,reason VARCHAR(200))");
 ...
 @Test void adjustmentsPreserveReservationsAndDeduplicate(){
  ...
  assertEquals(1,db.queryForObject("SELECT COUNT(*) FROM aster_catalog_change",Integer.class));
 ...
 @Test void rejectsStaleValuesAndReductionBelowLockedStock(){
  ...
  assertEquals(0,db.queryForObject("SELECT COUNT(*) FROM aster_catalog_change",Integer.class));
 }
